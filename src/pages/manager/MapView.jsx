...
import { supabase as sb, supabaseAdmin as sbAdm } from '../../supabase' 

// ── Status das escolas ────────────────────────────────────────
const STATUS_ESC = {
  nao_iniciada:       { cor:'#9CA3AF', label:'Sem OS',            emoji:'🏫' },
  programada:         { cor:'#3B82F6', label:'OS Aberta',         emoji:'📋' },
  em_execucao:        { cor:'#F59E0B', label:'Em execução',       emoji:'⚡' },
  aguardando_material:{ cor:'#EF4444', label:'Aguard. material',  emoji:'📦' },
  pendencia_retorno:  { cor:'#F97316', label:'Pendência/Retorno', emoji:'⚠️' },
  concluida:          { cor:'#22C55E', label:'Concluída',         emoji:'✅' },
  encerrada:          { cor:'#15803D', label:'Encerrada',         emoji:'🔒' },
}

function getStatusEscola(osLista) {
  if (!osLista || osLista.length === 0) return 'nao_iniciada'
  const abertas = osLista.filter(o => !['Concluída','Cancelada'].includes(o.status))
  if (abertas.length === 0) return 'concluida'
  if (abertas.some(o => o.status === 'Aguardando Material')) return 'aguardando_material'
  if (abertas.some(o => o.status === 'Em Execução'))         return 'em_execucao'
...
function makeEscolaIcon(L, status, nome) {
  ...
  const html = `
    ...
    ">
      ...
      ">${nome.length>18?nome.slice(0,18)+'…':nome}</div>
    </div>`
...
export default function MapView({ elecs, osList }) {
  ...
  // ── Geocodificar escolas e salvar coordenadas ─────────────────
  ...
  useEffect(() => {
    function init() {
      ...
      const ruas = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution:'© OSM', maxZoom:19, opacity:0.5 })
      const rua  = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution:'© OSM', maxZoom:19 })

      sat.addTo(map); ruas.addTo(map)

      L.control.layers(
        { '🛰 Satélite + Ruas': L.layerGroup([sat,ruas]), '🛰 Satélite': sat, '🗺 Mapa': rua },
  ...
  useEffect(() => {
    ...
    locs.forEach(loc => {
      ...
      const osElec = (osList||[]).filter(o => o.electrician_id === elec.id && !['Concluída','Cancelada'].includes(o.status))

      const popup = `<div style="font-family:Arial;min-width:200px;padding:4px">
        <b style="color:${st==='ativo'?'#1A478A':'#888'};font-size:14px">⚡ ${elec.name}</b><br>
        <span style="font-size:12px">${st==='ativo'?'🟢 Ativo':'st'==='inativo'?'🟡 Parado':'⚫ Offline'} · ${mins<1?'agora':mins+'min atrás'}</span><br>
        ${elec.phone?`<span style="font-size:11px;color:#666">📞 ${elec.phone}</span><br>`:''}
        ${osElec.length>0?`<hr style="margin:5px 0"><b style="font-size:11px">OS ativas: ${osElec.length}</b><br>`+''+osElec.map(o=>`<span style="font-size:10px">• ${o.number} — ${o.location?.name||'—'} (${o.status})</span><br>`).join(''):''}
        <span style="font-size:9px;color:#aaa">📍 ${loc.latitude.toFixed(5)}, ${loc.longitude.toFixed(5)}</span>
      </div>`
  ...
  useEffect(() => {
    ...
    comCoord.forEach(esc => {
      ...
      const osAbertas= osEsc.filter(o => !['Concluída','Cancelada'].includes(o.status))
      const status   = getStatusEscola(osEsc)
      const st       = STATUS_ESC[status] || STATUS_ESC.nao_iniciada
      const icon     = makeEscolaIcon(L, status, esc.name)

      // Eletricista responsável (da OS mais recente)
      ...
      const popup = `<div style="font-family:Arial;min-width:220px;padding:4px">
        <b style="color:#1A478A;font-size:13px">🏫 ${esc.name}</b><br>
        ${esc.address?`<span style="font-size:11px;color:#555">📍 ${esc.address}${esc.neighborhood?', '+esc.neighborhood:''}, Itabuna/BA</span><br>`:''}
        ${esc.nucleus?`<span style="font-size:11px;color:#888">Núcleo ${esc.nucleus}</span><br>`:''}
        ${esc.director?`<span style="font-size:11px;color:#555">👤 ${esc.director}</span><br>`:''}
        ${esc.phone?`<span style="font-size:11px;color:#555">📞 ${esc.phone}</span><br>`:''}
        ...
        ${elecResp?`<span style="font-size:11px;color:#555">⚡ Eletricista: <b>${elecResp.name}</b></span><br>`:''}
        ${osAbertas.length>0?`<span style="font-size:11px;color:#555">📋 ${osAbertas.length} OS aberta(s)</span><br>`:''}
        ${osEsc.filter(o=>o.status==='Concluída').length>0?`<span style="font-size:11px;color:#065F46">✅ ${osEsc.filter(o=>o.status==='Concluída').length} OS concluída(s)</span>`:''}
      </div>`
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem',flexWrap:'wrap',gap:8}}>
        <div>
          ...
          <p style={{fontSize:13,color:'#888780'}}>Escolas por status · Eletricistas em tempo real{lastUpd&&` · ${lastUpd.toLocaleTimeString('pt-BR')}`}</p>
        </div>
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          ...
          <button className="btn" onClick={fetchData} style={{fontSize:12}}>🔄</button>
        </div>
      </div>

      {erro && <div style={{background:'#FEE2E2',border:'0.5px solid #FCA5A5',borderRadius:8,padding:'10px 14px',marginBottom:'1rem',fontSize:13,color:'#991B1B'}}>⚠ {erro}</div>}
      ...
      {legenda && (
        <div style={{background:'#fff',border:'0.5px solid #e5e3dc',borderRadius:10,padding:'10px 14px',marginBottom:'1rem',display:'flex',flexWrap:'wrap',gap:16}}>
          ...
          <div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:4}}>
              {[['#1A478A','🟢 Ativo (GPS < 10min)'],['#F59E0B','🟡 Parado (10–60min)'],['#9CA3AF','⚫ Offline (> 60min)']].map(([cor,lbl]) => (
      ...
      {semCoord > 0 && !geocoding && (
        <div style={{background:'#FEF3C7',border:'0.5px solid #FCD34D',borderRadius:8,padding:'10px 14px',marginBottom:'1rem',fontSize:13,color:'#92400E',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexWrap:'wrap'}}>
          <span>⚠ <strong>{semCoord}</strong> escola(s) sem coordenadas · {comCoord} já no mapa</span>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:'1rem'}}>
        <div style={{background:ativos.length>0?'#D1FAE5':'#f5f5f4',borderRadius:8,padding:'0.7rem'}}>
          <p style={{fontSize:10,color:ativos.length>0?'#065F46':'#888780',marginBottom:2}}>🟢 Ativos agora</p>
        ...
        <div style={{background:'#f5f5f4',borderRadius:8,padding:'0.7rem'}}>
          <p style={{fontSize:10,color:'#888780',marginBottom:2}}>⚡ Com GPS</p>
          <p style={{fontSize:22,fontWeight:600}}>{locs.length}</p>
        </div>
        <div style={{background:'#E6F1FB',borderRadius:8,padding:'0.7rem'}}>
          <p style={{fontSize:10,color:'#0C447C',marginBottom:2}}>🏫 Escolas no mapa</p>
        ...
        <div style={{background:(contStatus.aguardando_material||0)>0?'#FEE2E2':'#f5f5f4',borderRadius:8,padding:'0.7rem'}}>
          <p style={{fontSize:10,color:(contStatus.aguardando_material||0)>0?'#991B1B':'#888780',marginBottom:2}}>📦 Aguard. material</p>
            ...
            <div key={e.id}
              ...
              <div>
                ...
                <p style={{fontSize:10,color:cor}}>
                  {!loc?'⚫ Sem GPS':st==='ativo'?`🟢 ${m<1?'agora':m+'min'}`:`${st==='inativo'?'🟡':'⚫'} ${m}min atrás`}
                </p>
              </div>
              {loc&&<span style={{fontSize:12}}>📍</span>}
            </div>
          )
        })}
      </div>
      ...
      <p style={{fontSize:11,color:'#888780',marginTop:6,textAlign:'center'}}>
        🛰 Satélite Esri · 🏫 {comCoord} escolas · ⚡ {locs.length} eletricistas · Atualiza a cada 30s
      </p>
    </div>
  )
}
